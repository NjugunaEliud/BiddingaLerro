...
export default function Top5Unique() {
    ...
    return (
        <div className='bg-gray-900'>
            <p className='text-2xl text-white font-semibold text-center'>Top 5 <span className='text-yellow-500'> Unique Bidders</span> </p>
            {loading ? (
                ...
                <div className='grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-4'>
                    {Object.keys(uniqueBiddersByProduct).map((productId, index) => (
                        <div key={index} className='border shadow shadow-yellow-500 rounded-md mb-6 p-4 m-4 ms-3 me-3'>
                            <p className='text-white text-xl font-semibold mb-2'>Product: {productId}</p>
